...
def validate_and_compare_to_clustering(model, env, distance, device, cfg):
    ...
    clst_metric_sp = ClusterMetrics()
    clst_metric_mcaff = ClusterMetrics()
    clst_metric_mcembed = ClusterMetrics()
    clst_metric_graphagglo = ClusterMetrics()

    map_rl, map_embed, map_sp, map_mcaff, map_mcembed, map_graphagglo, map_rl, map_rl = [], [], [], [], [], [], [], []
    ...
    for it in range(n_examples):
        ...
        ex_mc_gts.append(env.gt_soln[0].cpu())
        ...
        map_sp.append(segm_metric(clst_labels_sp_kmeans, gt_seg))
        clst_metric_sp(clst_labels_sp_kmeans, gt_seg)
    ...
    print(f"kmeans node : {np.array(map_sp).mean()}")
    print(f"kmeans pix  : {np.array(map_embed).mean()}")
    ...
    vi_kmnode, are_kmnode, arp_kmnode, arr_kmnode = clst_metric_sp.dump()
    vi_kmpix, are_kmpix, arp_kmpix, arr_kmpix = clst_metric.dump()
    ...
    print("\nVI: ")
    print(f"ours        : {vi_rl}")
    print(f"kmeans node : {vi_kmnode}")
    print(f"kmeans pix  : {vi_kmpix}")
    ...
    print("\nARE: ")
    print(f"ours        : {are_rl}")
    print(f"kmeans node : {are_kmnode}")
    print(f"kmeans pix  : {are_kmpix}")
    ...
    print("\nARR: ")
    print(f"ours        : {arr_rl}")
    print(f"kmeans node : {arr_kmnode}")
    print(f"kmeans pix  : {arr_kmpix}")
    ...
    for i in range(len(ex_gts)):
        fig, axs = plt.subplots(2, 5, figsize=(20, 10), sharex='col', sharey='row', gridspec_kw={'hspace': 0, 'wspace': 0})
        ...
        axs[0, 2].axis('off')
        axs[0, 3].imshow(ex_clst_graph_agglo[i], cmap=random_label_cmap(), interpolation="none")
        axs[0, 3].set_title('nagglo')
        axs[0, 3].axis('off')
